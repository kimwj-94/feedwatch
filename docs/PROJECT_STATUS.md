...
### 4-6. 대시보드 알림 처리 개선 (2026-07-25)

- **확인 즉시 정리** — 대시보드에서 새 글 제목을 열면 해당 글을 자동으로 읽음 처리
- **빠른 읽음 처리** — `내 알림` 각 행에 읽음 버튼, 카드 상단에 `모두 읽음` 제공
- **알림 더 보기** — 카드의 `더 보기`가 신규 목록의 `내 알림만` 필터로 연결
- **공유 상태 안내** — 모두 읽음 확인창에 읽은 글이 가족 공용 보관함으로 이동함을 명시
