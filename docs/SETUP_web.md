...
이제부터 다른 가족은 앱에서 한 번 **로그인 → 가입 신청** 하면 되고, 관리자가
**관리 → 가입 신청** 탭에서 **승인**하면 입장합니다. Firebase 권한은 로그인 계정의 UID를
기준으로 하므로 이메일만 미리 등록하는 방식은 사용하지 않습니다.
...
### 로그인 필요 사이트의 아이디/비밀번호·네이버 쿠키

**웹 화면에서 바로 등록합니다**(관리자만 가능). URL 관리에서 유형을 `로그인 필요`로 고르면 아이디·비밀번호
칸이 나타나고, `네이버 카페/블로그`를 고르면 비공개 카페용 쿠키 칸이 나타납니다.

1. 아이디/비밀번호 또는 네이버 쿠키를 입력하고 저장하면 **‘수집 비밀번호’** 를 한 번 묻습니다(직접 정하는 값).
...
목록에는 "로그인정보 등록됨/미등록"으로 표시됩니다. 비밀번호나 쿠키를 바꾸려면 다시 입력해 저장하면 교체됩니다.
