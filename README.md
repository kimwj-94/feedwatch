...
- 접근 통제: **관리자가 승인한 계정만** 입장합니다.
  ...
  - 관리자는 **관리자 → 가입 신청** 탭에서 승인/거절합니다. (이메일을 미리 등록하려면 사용자 관리에서 추가)
...
| `FEEDWATCH_CRED_PASSPHRASE` | (로그인 필요 사이트가 있을 때) 웹에서 정한 ‘수집 비밀번호’ |
...
- **로그인 사이트 자격증명**: 웹 관리자(관리자 권한)가 **URL 관리 화면에서 아이디/비밀번호를 직접 입력**합니다.
  ...
  크롤러는 같은 값(`FEEDWATCH_CRED_PASSPHRASE`)으로 복호화합니다. 평문/암호화 키는 브라우저·DB에 남지 않습니다.
...
- 보안 규칙([`firestore.rules`](firestore.rules)): 로그인한 가족만 읽기, **회원은 항목 상태값만 변경**,
  소스/구분값/사용자/설정 쓰기는 **관리자만**, `credentials`는 **클라이언트 완전 차단**(크롤러 Admin SDK 전용).
